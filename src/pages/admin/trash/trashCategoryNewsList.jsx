...
const TrashCategoryNewsList = () => {
    ...
    // danh sách danh mục
    const fetchTrashNewsCategory = async (page = 1) => {
        setLoading(true);
        try {
            const response = await laravelAPI.get("/api/admin/trash/newsCategory", {
                params: {
                    page: page,
    ...
    useEffect(() => {
        fetchTrashNewsCategory(currentPage);
    }, [currentPage]);
    ...
    // /xóa vĩnh viễn
    const handleForceDeleteNewsCategory = async (newsCategoryId) => {
        Swal.fire({
            title: "Bạn có chắc chắn?",
            text: " Danh mục bài viết này sẽ bị Xóa vĩnh viễn.",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                try {
                    const response = await laravelAPI.delete(
                        `/api/admin/newscategories/${newsCategoryId}`
                    );
    ...
    // Khôi phục
    const handleRestoreNewsCategory = async (newsCategoryId) => {
        Swal.fire({
            title: "Bạn có chắc chắn?",
            text: "Danh mục bài viết này sẽ được khôi phục.",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            ...
            if (result.isConfirmed) {
                try {
                    const response = await laravelAPI.patch(
                        `/api/admin/newscategories/restore/${newsCategoryId}`
                    );
                    if (response.code === "success") {
                        Swal.fire({
                    ...
                    console.error("Lỗi khi khôi phục Danh mục bài viết:", error);
                    Swal.fire({
                        title: "Error!",
                        text: "Khôi phục không thành công.",
                        icon: "error",
    ...
    return (
        <div className="py-[60px]">
            <h2 className="text-[28px] font-[700] text-[#00000] text-center pb-[30px]">Quản lý danh mục bài viết</h2>
                                            ...
                                            <tr key={category.id}>
                                                ...
                                                <td>
                                                    <div className="flex items-center justify-center gap-[6px] !py-[20px]">
                                                        <button
                                                            onClick={() => handleRestoreNewsCategory(category?.id)}
                                                            className="text-[16px] font-[600] text-[#ffffff] bg-[#FFCC00] rounded-[8px] py-[8px] px-[12px]"
                                                        >
                                                            Khôi phục
                                                        </button>
                                                        <button
                                                            onClick={() => handleForceDeleteNewsCategory(category?.id)}
...
export default TrashCategoryNewsList;