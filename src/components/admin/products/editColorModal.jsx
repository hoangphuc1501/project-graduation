...
const EditModalColor = (props) => {
    const { showEditColor, setShowEditColor, colorId, refreshColor  } = props;
    const handleClose = () => setShowEditColor(false);
    ...
    useEffect(() => {
        if (showEditColor && colorId) {
            fetchSizeDetails(colorId);
        }
    }, [showEditColor, colorId]); 

    const fetchSizeDetails = async (id) => {
        try {
            const response = await laravelAPI.get(`/api/admin/colors/${id}`);
    ...
    const handleUpdateSize = async (e) => {
        ...
        try {
            const response = await laravelAPI.patch(`/api/admin/colors/${colorId}`, {
                name,
            ...
            >
                ...
                <Modal.Body>
                    <form onSubmit={handleUpdateSize}>
                        <h2 className='text-[24px] text-[#000000] font-[700] text-center pb-[20px]'>cập nhật màu sắc</h2>
                        <div className='px-[10px] mb-[20px]'>
                            <label className='text-[18px] text-[#000000] font-[700] pb-[8px]' >Tên màu sắc</label>
